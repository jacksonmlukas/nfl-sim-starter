...
import argparse, json, numpy as np, pandas as pd, torch, torch.nn.functional as F
...
def top_p_sample(logits, legal_set, top_p=0.85):
    ...
    for i, pt in enumerate(PLAY_TYPES):
        if pt in legal_set: mask[i] = 0.0
    logits = logits + mask
    probs = F.softmax(logits, dim=-1).cpu().numpy()
    idx = np.argsort(probs)[::-1]
    cum = probs[idx].cumsum()
    keep = idx[cum <= top_p]
    if len(keep) == 0: keep = idx[:2]
...
def punt_bias(yl100: int) -> float:
    ...
    if yl100 >= 80: return 0.7
    if yl100 >= 70: return 0.4
    if yl100 >= 60: return 0.2
    return 0.0

def sample_from_quantiles(q):
    q = np.sort(np.asarray(q, dtype=float))
    u = np.random.rand(); k = min(int(u*9), 8)
    if k == 0: lo, hi = -10.0, q[0]
    elif k == 8: lo, hi = q[8], 80.0
    else: lo, hi = q[k-1], q[k]
    if hi < lo: lo, hi = hi, lo
    if hi - lo < 1e-3: return float(np.clip(hi, -10, 80))
    return float(np.clip(np.random.uniform(lo, hi), -10, 80))

def time_bucket(gs): return int(np.clip(gs // 30, 0, 120))
def score_bucket(sd): sd=int(np.clip(sd,-28,28)); return (sd+28)//2
def choose_flags(down:int, ytg:int, gs:int):
    shotgun_p = 0.7 if (down==3 and ytg>=7) else (0.55 if ytg>=8 else 0.35)
    if gs <= 120: shotgun_p = max(shotgun_p, 0.6)
...
def main():
    ...
    team_default = (args.home_team_code, args.away_team_code)
    try:
        vocab = json.load(open("runs/vocab.json"))
    ...
    for g in range(args.n_games):
        s = State(qtr=1, game_seconds_remaining=3600, down=1, ydstogo=10, yardline_100=75,
                  posteam_timeouts=3, defteam_timeouts=3, possession='home', score_home=0, score_away=0)
        drives = 0
        K = args.context_k

        def cur_teams():
            return (args.home_team_code, args.away_team_code) if s.possession=='home' else (args.away_team_code, args.home_team_code)
        ...
        while s.game_seconds_remaining > 0 and drives < 24:
            ...
            if s.down == 3 and s.ydstogo >= 7: logits[PASS] += 1.0; logits[RUN] -= 0.3
            if s.down == 2 and s.ydstogo >= 8: logits[PASS] += 0.3
            if s.down == 1 and s.ydstogo == 10: logits[PASS] += 0.2
            if s.ydstogo <= 2: logits[RUN] += 0.2
            if s.game_seconds_remaining > 60: logits[SPIKE] -= 1.0
            ...
            if play_type in ("run","pass"):
                ...
                flip_possession(s, new_yard_old_offense); drives += 1
            elif play_type == "field_goal":
                kd = s.yardline_100 + 17
                if np.random.rand() < fg_make_prob(kd):
                    if s.possession == 'home': s.score_home += 3
                    else: s.score_away += 3
            ...
            # clock & clamp
            s.game_seconds_remaining = max(0, s.game_seconds_remaining - (38 if play_type in ("run","kneel") else 28))
            s = clamp_state(s)
            ...
            p, d = cur_teams()
            posteam_buf.append(p); defteam_buf.append(d)
            season_buf.append(season_buf[-1])
            roof_buf.append(roof_buf[-1]); surface_buf.append(surface_buf[-1])
            temp_buf.append(temp_buf[-1]); wind_buf.append(wind_buf[-1])
            g2g_buf.append(int((s.yardline_100 <= 10) and (s.ydstogo >= s.yardline_100)))
            rz_buf.append(int(s.yardline_100 <= 20))

            rows.append({'game': g, 'qtr': s.qtr, 'secs': s.game_seconds_remaining, 'down': s.down,
                         'ydstogo': s.ydstogo, 'yardline_100': s.yardline_100, 'play_type': play_type,
                         'yards': float(yards), 'score_home': s.score_home, 'score_away': s.score_away})
