...
import argparse, yaml, os, json, random, numpy as np, pandas as pd, torch
...
def yard_bucket(y): return int(np.clip(y, 0, 100))
def dist_bucket(d): d=int(max(1,min(99,int(d)))); return d if d<=20 else 21
def time_bucket(gs): return int(np.clip(int(gs) // 30, 0, 120))
def score_bucket(sd): sd=int(np.clip(int(sd),-28,28)); return (sd+28)//2
def flag_bucket(row):
...
def bool_bucket(b): return 1 if bool(b) else 0
...
class PlaysDataset(Dataset):
    def __init__(self, parquet_path, sample_plays=None, context_k=8):
        ...
        # Normalize labels
        df["play_type_norm"] = df["play_type"].astype(str).replace({"qb_spike":"spike","qb_kneel":"kneel"})
        ...
        for col in ["posteam","defteam","season"]:
            if col not in df: df[col] = "UNK"
            df[col] = pd.Categorical(df[col])
            df[f"{col}_code"] = df[col].cat.codes.astype("int64")

        # Environment (fill if missing)
        if "roof" not in df: df["roof"] = np.full(len(df), "outdoors")
        if "surface" not in df: df["surface"] = np.full(len(df), "grass")
        ...
        # Context flags
        df["g2g_flag"] = ((df["ydstogo"] == df["yardline_100"]) & (df["yardline_100"] <= 10)).astype(int)
    ...
    def __len__(self): return max(0, len(self.df)-self.k-1)
    ...
    def __getitem__(self, idx):
        ...
        x = {
            ...
            'yard_bucket':  torch.tensor(ctx['yardline_100'].apply(yard_bucket).values, dtype=torch.long),
            ...
            'time_bucket':  torch.tensor(ctx['game_seconds_remaining'].apply(time_bucket).values, dtype=torch.long),
            'score_bucket': torch.tensor(ctx['score_differential'].apply(score_bucket).values, dtype=torch.long),
...
def collate(batch):
    keys = ['down','yard_bucket','dist_bucket','flags','time_bucket','score_bucket',
            'posteam','defteam','season','roof_bucket','surface_bucket','temp_bucket',
            'wind_bucket','g2g_flag','rz_flag']
    xs = {k: [] for k in keys}
    ys_pt, ys_yards = [], []
    for x,y in batch:
        for k in keys: xs[k].append(x[k])
        ys_pt.append(y['pt']); ys_yards.append(y['yards'])
...
def main():
    ...
    model = PlayTransformer(
        d_model=cfg.model.d_model, n_layers=cfg.model.n_layers, n_heads=cfg.model.n_heads, dropout=cfg.model.dropout,
        n_play_types=len(PLAY_TYPES),
        n_team=max(ds.team_card, 40), n_season=max(ds.season_card, 16),
        n_roof=max(ds.roof_card, 5), n_surface=max(ds.surface_card, 6),
        max_len=args.context_k
    ).to(device)

    opt = torch.optim.AdamW(model.parameters(), lr=cfg.train.lr, weight_decay=cfg.train.weight_decay, betas=(0.9,0.95))
    ...
    for epoch in range(999):
        for xb, y_pt, y_yards in dl:
            ...
            y_pt = y_pt.to(device); y_yards = y_yards.to(device)
            ...
            wt = wt * torch.where(is_3rd7, torch.tensor(2.0, device=device), torch.tensor(1.0, device=device))
            wt = wt * torch.where(is_2min, torch.tensor(1.3, device=device), torch.tensor(1.0, device=device))

            # play-type loss
            ce = F.cross_entropy(out['play_type_logits'], y_pt, weight=cw, label_smoothing=0.03, reduction='none')
            loss_pt = (ce * wt).mean()
            ...
            if m4.any():
                ...
                loss_fg   = F.binary_cross_entropy_with_logits(out['fg_attempt_logit'][m4], fg_target[m4])
                loss_punt = F.binary_cross_entropy_with_logits(out['punt_attempt_logit'][m4], punt_target[m4])
            else:
            ...
            steps += 1
            if steps > warmup: sch.step()
            if steps % 100 == 0:
                print(f"step {steps} | loss_pt {loss_pt.item():.4f} | loss_y {loss_y.item():.4f} | aux {(loss_fg+loss_punt).item():.4f}")
            if steps >= cfg.train.max_steps: break
        if steps >= cfg.train.max_steps: break
