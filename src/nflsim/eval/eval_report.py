...
def main():
    ...
    if len(rp_sim) > 0 and len(rp_real) > 0:
        plot_hist_overlay(
            ...
            title="Yards/Play (Run+Pass, clipped [-10,80])",
            out_png=yards_png,
        )
    ...
    with open(args.out, "w") as f:
        ...
        f.write("## Notes\n")
        f.write(
            "- Early prototype uses simple state updates and median-yards proxy; expect drift.\n"
        )
        f.write(
            "- Next step: train real targets (play_type + yards quantiles) and feed them into the decoder.\n"
        )
