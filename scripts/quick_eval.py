...
def norm_pt(pt):
    ...
    if pt and pt.startswith("field_goal"): return "field_goal"
    return pt
...
def stats(x):
    ...
    return dict(n=len(x), mean=float(x.mean()), std=float(x.std()),
                p10=float(x.quantile(.1)), p50=float(x.quantile(.5)), p90=float(x.quantile(.9)))
...
def main():
    ...
    sim_path = args.sims or (sorted(glob.glob("runs/sim_*games.parquet"))[-1] if glob.glob("runs/sim_*games.parquet") else "")
    if not sim_path:
        print("No sim parquet found in runs/ (expected runs/sim_*games.parquet)"); sys.exit(1)
    ...
    print(pt.to_string()); print("max_abs_diff:", round(pt["abs_diff"].max(), 3), "\n"); sys.stdout.flush()
    ...
    s = sims.loc[sims["play_type"].isin(["run","pass"]), "yards"].clip(-10,80) if "yards" in sims else pd.Series(dtype=float)

    print("-- Yards/play (run+pass, clipped [-10,80]) --")
    from pprint import pprint
    print("real:"); pprint({k: (round(v,3) if isinstance(v,(int,float,np.floating)) else v) for k,v in stats(r).items()})
    print("sim: "); pprint({k: (round(v,3) if isinstance(v,(int,float,np.floating)) else v) for k,v in stats(s).items()}); print()
    sys.stdout.flush()
